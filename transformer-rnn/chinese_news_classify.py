...
def train_model(model, train_dataset, val_dataset, batch_size=16, epochs=3, learning_rate=3e-5, device='cuda'):
    ...
    # 打开文件用于写入评价指标
    with open("chinese_metrics_log.txt", "a") as metrics_file:
...
if __name__ == "__main__":
    # 数据文件路径
    file_path = '../chinese_news.xlsx'  # 替换为你的数据文件路径

    # 加载数据
    train_dataset, val_dataset = load_data(file_path)

    # 初始化模型和分词器
    model = BertGRUClassifier(bert_model_path='../pretrained_models/bert-base-chinese')
    tokenizer = BertTokenizer.from_pretrained('../pretrained_models/bert-base-chinese')
    ...
    # 保存训练好的模型
    save_model(model, tokenizer, './chinese_news_classifier')
